...
impl Mesh {
    pub fn new(vertices: &[f32], normals: &[f32], indices: &[u32]) -> Result<Mesh> {
        ...
        let vertices = indices.iter().map(|i| i * 3).map(|i| [i, i + 1, i + 2]).flatten().map(|i| vertices[i as usize]).collect();
        let normals = indices.iter().map(|i| i * 3).map(|i| [i, i + 1, i + 2]).flatten().map(|i| normals[i as usize]).collect();
...
impl MeshCanvas {
    pub fn new(canvas: web_sys::HtmlCanvasElement) -> Result<MeshCanvas, JsValue> {
        ...
        let world_view_projection_location = gl.get_uniform_location(&program, "u_worldViewProjection");
        let world_inverse_transpose_location = gl.get_uniform_location(&program, "u_worldInverseTranspose");
        let color_location = gl.get_uniform_location(&program, "u_color");
        let reverse_light_direction_location = gl.get_uniform_location(&program, "u_reverseLightDirection");
    ...
    pub fn draw(&self) {
        ...
        {
            self.gl.enable_vertex_attrib_array(self.position_location as u32);
        ...
        {
            self.gl.enable_vertex_attrib_array(self.normal_location as u32);
            self.gl.bind_buffer(WebGl2RenderingContext::ARRAY_BUFFER, Some(&self.normal_buffer));
        ...
        let zNear = 1.0;
        let zFar = 2000.0;
        let projection_matrix = na::Matrix4::new_perspective(fieldOfView, aspect, zNear, zFar);
        let eye = na::Point3::new(200., 50., -500.);
        let target = na::Point3::new(-5.6, -13.1, 0.);
        let up = na::Vector3::new(0., 1., 0.);
        let camera_matrix = na::Matrix4::look_at_rh(&eye, &target, &up);
        let view_matrix = camera_matrix.try_inverse().unwrap();
        let view_projection_matrix = projection_matrix * view_matrix;
        let world_matrix = na::Matrix4::identity();
        ...
        self.gl.uniform_matrix4fv_with_f32_array(self.world_view_projection_location.as_ref(), false, world_view_projection_matrix.as_slice());
        self.gl.uniform_matrix4fv_with_f32_array(self.world_inverse_transpose_location.as_ref(), false, world_inverse_transpose_matrix.as_slice());
        self.gl.uniform4f(self.color_location.as_ref(), 0.2, 1.0, 0.2, 1.0);
        self.gl.uniform3f(self.reverse_light_direction_location.as_ref(), 0.5, 0.7, 1.0);


        let count = 16 * 6;
        let offset = 0;
        self.gl.draw_arrays(WebGl2RenderingContext::TRIANGLES, offset, count);
...
-1, 0, 0,
-1, 0, 0        
    ].iter().map(|x| *x as f32).collect::<Vec<_>>();
...
fn set_geometry(gl: &WebGl2RenderingContext) {
    let geometry=&[
         ...
         30,   0,  0,
          ...
          0,   0,   0,
          0, 150,  30,
          0, 150,   0,][..].chunks(3).map(|p| [p[0] - 50, p[1] - 75, p[2] -15]).flatten().map(|x| x as f32 ).collect::<Vec<_>>();
          let vs = geometry[..].chunks(3).map(|p| na::Vector3::new(p[0], p[1], p[2])).collect::<Vec<_>>();
          let mean_v = vs.iter().sum::<na::Vector3<f32>>() / vs.len() as f32;
          info!("mean: {:?}", mean_v);
